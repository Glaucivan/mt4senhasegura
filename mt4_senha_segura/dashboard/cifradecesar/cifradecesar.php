...
<html lang="pt-BR">
  ...
  <body id="page-top">
    ...
    <div id="wrapper">
      ...
      <ul class="sidebar navbar-nav">
		...
		<li class="nav-item dropdown">
          ...
          <div class="dropdown-menu" aria-labelledby="pagesDropdown">
            <a class="dropdown-item" href="cifradecesar.php">Criptografar/<br>Descriptografar</a>
          </div>
        </li>
	...
	<div class="container">
        <div class="card-body">
          <form method="GET" id="cadastro">
			...
			<input type="submit" class="btn btn-primary btn-block" value="CRIPTOGRAFAR E DESCRIPTOGRAFAR">
			
			<?php
	//Define o tamanho do alfabeto
	$tamanhoAlfabeto=256;
	//Define quantos algarismos iremos deslocar
	$n=4;
	// Define quantos caracteres ficarão de fora (0 - x - 1)
	$fora = 32;
	...
	for ($i = 0; $i < strlen($aCriptografar); $i++){
		//Número de casas estipulado
		$key = ord($aCriptografar[$i]);
		
		//Avança o número de casas estipulado
		$novoCodigo = $key + $n;
		
		//Calcula o código de caracteres
		$novoCodigo = $novoCodigo % $tamanhoAlfabeto;
		
		// Se o novo código estiver na faixa de exclusão,
		// empurra-o para fora dessa faixa
			if($novoCodigo >= 0 && $novoCodigo < $fora){
				$novoCodigo += $fora; 
			}			
			
			//Obtém o caractere do novo código e concatena-o
		$criptografada .=chr($novoCodigo);		
		}
		
		//Varre todos os caracteres da frase criptografada
		for($i =0; $i < strlen($criptografada); $i++ ){
			//Obtém o código do caractere atual
			$key = ord($criptografada[$i]);
			
			// Retrocede o número de casas estipulado
			$novoCodigo =$key - $n;
		...
		// Computa a diferença caso o valor seja negativo
		if($novoCodigo < 0){
			// Ficar atento no jogo de sinal
			$novoCodigo = $tamanhoAlfabeto +$novoCodigo;
		}
		
		// Obtém o caracteredo novo código
		...
		echo "<h1 class='badge badge-dark'>FRASE CRIPTOGRAFADA:</h1> <p class='badge badge-success'>".$criptografada."</p><br>";
		echo "<h1 class='badge badge-dark'>FRASE DESCRIPTOGRAFADA:</h1> <p class='badge badge-warning'>".$desCriptografada."</p><br>";
